describe('Shopify#blog', () => {
  ...
  it('gets a count of all blogs', () => {
    ...
    scope.get('/admin/blogs/count.json').reply(200, output);

    return shopify.blog.count().then((data) => expect(data).to.equal(2));
  });

  it('gets a single blog by its ID (1/2)', () => {
    const output = fixtures.res.get;

    scope.get('/admin/blogs/241253187.json').reply(200, output);
  ...
  it('gets a single blog by its ID (2/2)', () => {
    ...
    scope.get('/admin/blogs/241253187.json?foo=bar').reply(200, output);

    return shopify.blog
      .get(241253187, { foo: 'bar' })
      .then((data) => expect(data).to.deep.equal(output.blog));
  });
  ...
  it('creates a new blog', () => {
    ...
    scope.post('/admin/blogs.json', input).reply(201, output);
  ...
  it('updates a blog', () => {
    ...
    scope.put('/admin/blogs/241253187.json', input).reply(200, output);

    return shopify.blog
      .update(241253187, input.blog)
      .then((data) => expect(data).to.deep.equal(output.blog));
  });

  it('deletes a blog', () => {
    scope.delete('/admin/blogs/241253187.json').reply(200, {});
