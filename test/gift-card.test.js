describe('Shopify#giftCard', () => {
  ...
  it('gets a list of all gift cards (1/2)', () => {
    ...
    scope.get('/admin/gift_cards.json').reply(200, output);

    return shopify.giftCard
      .list()
  ...
  it('gets a list of all gift cards (2/2)', () => {
    ...
    scope.get('/admin/gift_cards.json?status=enabled').reply(200, output);
  ...
  it('gets a single gift card by its ID', () => {
    ...
    scope.get('/admin/gift_cards/48394658.json').reply(200, output);
  ...
  it('gets a count of all gift cards (1/2)', () => {
    scope.get('/admin/gift_cards/count.json').reply(200, { count: 2 });

    return shopify.giftCard.count().then((data) => expect(data).to.equal(2));
  });
  ...
  it('creates a new gift card', () => {
    ...
    scope.post('/admin/gift_cards.json', input).reply(201, output);

    return shopify.giftCard
      .create(input.gift_card)
      .then((data) => expect(data).to.deep.equal(output.gift_card));
  });
  ...
  it('updates a gift card', () => {
    ...
    scope.put('/admin/gift_cards/48394658.json', input).reply(200, output);

    return shopify.giftCard
      .update(48394658, input.gift_card)
      .then((data) => expect(data).to.deep.equal(output.gift_card));
  });
  ...
  it('searches for gift cards matching a given query', () => {
    ...
    scope.get('/admin/gift_cards/search.json?query=Bob').reply(200, output);
